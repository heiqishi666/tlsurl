...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path in ("/gzip", "/deflate", "/br", "/zstd"):
            # br/zstd fixtures generated by Node 24 zlib from 2048 ASCII x bytes.
            body = {"/gzip": gzip.compress(b"x" * 2048), "/deflate": zlib.compress(b"x" * 2048),
                    "/br": base64.b64decode("G/8H+CXwwrFAIHc="),
                    "/zstd": base64.b64decode("KLUv/WAAB00AABB4eAEA+/8ACw==")}[self.path]
            headers.append(("Content-Encoding", self.path[1:]))
...
def check_sync(base):
    ...
    for encoding in ("gzip", "deflate", "br", "zstd"):
        assert client.get(base + "/" + encoding).body == b"x" * 2048
        expect_error(lambda: tlsurl.Client(max_response_bytes=1024).get(base + "/" + encoding), "BODY_TOO_LARGE")
    assert tlsurl.Client(decompress=False).get(base + "/gzip").body.startswith(b"\x1f\x8b")
    expect_error(lambda: tlsurl.Client(max_response_bytes=1024).get(base + "/gzip"), "BODY_TOO_LARGE")
